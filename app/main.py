...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        with col2:
            # Reset to defaults
            if st.button("🔄 Reset", help="Reset all settings to defaults"):
                st.session_state.config = ScoringConfig.get_default()
                st.rerun()
        ...
        with st.expander("🏷️ Themes & Keywords"):
...
def render_main_content():
    ...
    st.markdown(
        '<p class="sub-header">Score and rank LinkedIn profiles for innovation roundtable events</p>',
        unsafe_allow_html=True
    )
    ...
    if uploaded_file is not None:
        ...
        with col2:
            preview_data = st.button("👁️ Preview Data", use_container_width=True)
    ...
    if st.session_state.stats is not None:
        ...
        with col4:
            st.metric("⚠️ Company Failures", stats["top_n_company_failures"])
        
        # Event info
        with st.expander("ℹ️ Scoring Configuration Used"):
